...
export const brapiMetaAnalysis = prompt('brapi_meta_analysis', {
  ...
  generate: (args) => {
    ...
    const text = [
      ...
      '2. From `distributions.studyName`, capture the distinct studies returning observations. If `hasMore` is true and a `dataset` handle is returned, page through with `brapi_manage_dataset` (mode: `load`) to materialize the full set.',
      '3. For each study, call `brapi_get_study` to capture program, location, season, and trial context — these are the moderators in the meta-analysis.',
      '',
