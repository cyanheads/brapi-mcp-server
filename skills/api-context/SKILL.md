...
The framework's preferred pattern — when servers want both:

- Tokens shared across sessions (e.g. `df_<uuid>` handed from Agent A to Agent B) resolve on the receiving side when the server routes direct lookups against a shared backing store. The lookup key is the token, not the session.
- Session-scoped *enumeration* (e.g. `dataframe_describe` returning only items registered by the current session) is a per-server pattern: maintain a session-keyed registry of known names, gate list-all on it, but route direct lookups against the shared backing store.

Servers may instead choose stricter isolation by carving a separate backing store per session — direct token lookup then does not cross session boundaries. `brapi-mcp-server` under `BRAPI_SESSION_ISOLATION=true` (default) takes this path: each session gets its own canvas, so `df_<uuid>` resolves only within the session that registered it. `BRAPI_SESSION_ISOLATION=false` reverts to one shared tenant canvas, restoring the cross-session token-lookup semantics above.
